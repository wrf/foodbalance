...
fooddatafile = "./data/FoodBalanceSheets_E_All_Data_2023-09-06.csv.gz"
...
fooddata_recode = fooddata %>%
                  ...
                  select(Area, Item, Element, Y2011, Y2012, Y2013, Y2014, Y2015, Y2016, Y2017, Y2018, Y2019, Y2020 ) %>%
                  rename(region=Area )
...
worldpolygons$region = recode(worldpolygons$region,
                              "Turkey" = "Türkiye",
...
ui <- fluidPage(
  ...
  verticalLayout(
    fluidRow(
      column(4,
             # sliderInput(inputId = "lat",  label = "Latitude",
             #             min = -90, max = 90, value = c(-50,70)
             #            ),
             # sliderInput(inputId = "long", label = "Longitude",
             #             min = -180, max = 180, value = c(-180,180)
             #            ),
             selectInput("year", "Year (on map and plot)", 
                         choices = list("2020" = "Y2020",
                                        "2019" = "Y2019",
                                        "2018" = "Y2018",
                                        "2017" = "Y2017",
                                        "2016" = "Y2016",
    ...
    mainPanel(width="100%",
              ...
              a("https://github.com/wrf/foodbalance", href="https://github.com/wrf/foodbalance"),
              br(), br(),
              p("For all measures, units are in 1000 tonnes, kilotonnes, or M kg, or properly in gigagrams - Gg. 
...
server <- function(input, output) {
  ...
  mapgg = reactive({
    ...
    ggplot(fooddata_subset, aes(long,lat, group=group)) +
      ...
      labs(x=NULL, y=NULL, fill="Quantity\n(M kg)",
           ...
           caption="Data from UN Food and Agriculture Organization (FAO) 2023\nwww.fao.org/faostat/en/#data/FBS") +
  ...
  output$printpdf <- downloadHandler(
    ...
    content = function(filename){
      printmap = mapgg()
      ggsave(filename, printmap, device="pdf", 
             title=paste( input$itemType, input$elementType, input$year), 
             width=11, height=6)
    }
  )
